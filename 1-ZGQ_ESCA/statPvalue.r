...
for (i in cynames) {
  ...
  if (num < 2 ) {
    ...
    p<- ggplot(cydf, mapping = aes(x=Type, y=measurement)) + 
      geom_boxplot() + 
      annotate(geom="text", label=pvalue, color="red") +
      ylab(i) +
...
library(ggpubr)
cydf2 <- na.omit(cydf2)
cydf2 <- subset(cydf2,Path!="NA")
p <- ggboxplot(cydf2, x = "Path", y = "measurement",
               color = "Type", palette = "jco",
               add = "jitter")
p + ylab(cydf2$cytokins[1])+ stat_compare_means(aes(group = Type), method = "t.test")
